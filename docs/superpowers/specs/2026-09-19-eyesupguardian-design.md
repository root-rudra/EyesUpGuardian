...
6. **Targets:** idle CPU < 0.1% (60 s average), idle memory footprint ≤ 30 MB (`phys_footprint`; RSS is not used because it counts shared system frameworks, about 75 MB even for an empty SwiftUI app), and dashboard-open CPU < 1.5%. **The idle target describes the app at rest in its default configuration** (menu-bar icon and time left, no window, no HUD), which `make perf` enforces and which measures 0.000%. Visible, user-requested displays fall under the 1.5% target instead: measured on the reference Mac, the menu-bar stat readout costs ~0.4% of one core and a pinned HUD 0.30% idle (~1.1% while counting down), because writing changing text into the menu bar or a floating panel forces a re-layout each time. The menu-bar countdown ticks every second while a session is running (Settings can make it whole
minutes instead, which measures 0.000%). Ticking costs 0.5-0.7% of a core, but only because the
title is padded to a fixed width: letting it change width as digits come and go measured 2.2%,
since the whole menu bar re-lays out each time. Plan 3 measured the rest by bisection; the figures
live in the manual checklist. Measured with the
dashboard open on the reference Mac: **Overview 1.5-1.7%** (headline numbers every second, the GPU,
disk, network and sensor probes every five, sparklines drawn from at most 120 points) and
**Processes 1.3%** at its default five-second refresh — the same table at one second costs about
5%, which is why the rate is the user's choice and defaults to what Activity Monitor uses. Metrics
are sampled per metric rather than per tick, so a menu-bar readout asking for CPU every two seconds
no longer drags the process list along with it. `make perf-stats` measures the visible configuration end to end (HUD pinned, CPU and power in the menu bar) against the 1.5% limit, putting your own settings back afterwards; it measures 0.750%.
